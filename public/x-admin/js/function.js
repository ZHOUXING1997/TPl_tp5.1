...
* @params function  errorCallBack   失败回调函数
* */
function ajaxCommon(url, data, method = 'post', succCallBack, errorCallBack) {
    $.ajax({
        url: url,
        type: method,
        data: data,
        ...
        var uploadInst = upload.render({
            ...
            choose: function (obj) {
                ...
                if (res.code == 0) {
                    ...
                    $(elem).parent('.layui-upload').children('div:first').find('input:first').val(res.data.id); //图片链接（base64）
                } else {
        ...
        var uploadListIns = upload.render({
            ...
            , done: function (res, index, upload) {
                if (res.code == 0) { //上传成功
                    ...
                    // 插入form数据
                    tds.eq(4).append(`<input type="hidden" name="${formKey}[]" value="${res.data.id}">`); //清空操作
            ...
            , error: function (index, upload) {
                ...
                tds.eq(3).find('.up-file-reload').removeClass('layui-hide'); //显示重传
            }
        });
    });
}
    ...
    if (length <= 0) {
        ...
        data.forEach(function(v,i){
            ...
            var tr = $([
                `<tr id="upload-${i}">
                     <td>${v.filename}</td>
                     <td><img height="80px" src="${v.file_path}" alt="${v.filename}" class="layui-upload-img"></td>
                     <td>${(v.file_size / 1014).toFixed(1)}kb</td>
                     <td><span class="up-image" data-up="true">已上传</span></td>
                     <td class="td-action">
                        <button type="button" class="layui-btn layui-btn-xs layui-btn-danger img-delete">删除</button>
                        ${moveStr}
                        <input type="hidden" name="${formKey}[]" value="${v.id}">
                     </td>
                 </tr>`
            ].join(''));
