...
class Pay {
    ...
    public static function getPayMoney($status, $price, $uid) {
        ...
        if (empty($status)) {
            Log::record('release getPayMoney white user list: ' . json_encode(self::DEBUG_PAY_WHITE_LIST), Log::NOTICE);
        ...
        foreach ($prefixes as $prefix) {
            if (strpos($host, $prefix) !== false) {
                Log::record('release getPayMoney white: uid[$uid] price[$price] DEBUG_PAY_REALM_NAME ' . json_encode(self::DEBUG_PAY_REALM_NAME), Log::NOTICE);
                return self::DEBUG_PAY_PRICE;
            }
